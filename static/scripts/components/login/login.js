...
class Login extends React.Component{
    ...
    getLogin(){
        if (findDOMNode(this.refs.username).value==""||findDOMNode(this.refs.code).value=="") {
    ...
    getCode(){
        ...
        if (test) {
            ...
            UserAction.getUserCode({phone:findDOMNode(this.refs.username).value,type:'forget'});
        }else{
                        ...
                        <li>
                            <div className="codeBtn">
                                ...
                                <input
                                    ...
                                    maxLength="6"
                                />
                                <span ref="getCode" className="getCode" onClick={this.getCode}>获取验证码</span>
                            </div>
                        </li>
                        <li className="voice">长时间收不到验证码，可尝试<u>语音接听验证码</u></li>
                        <li>
                            <button className="loginBtn" onClick={ this.getLogin }>进入</button>
                        </li>
                    </ul>
                    <footer>
                        <span className="loginFooter">用户协议</span>
                        <span className="loginFooter" onClick={this.passwordLogin}>密码登录</span>
                    </footer>
                </div>
            </div>
        )
